...
class ASDiffusionModel(nn.Module):
    ...
    def model_predictions(self, backbone_feats, x, t):
        ...
        x_start, _ = self.decoder(backbone_feats, t, x_m.float()) # torch.Size([1, C, T])
    ...
    def forward(self, backbone_feats, t, event_diffused, event_gt=None, boundary_gt=None): # only for train
        ...
        if cond_type == 'full':
            event_out, frame_feat = self.decoder(backbone_feats, t, event_diffused.float())
        
        elif cond_type == 'zero':
            event_out, frame_feat= self.decoder(torch.zeros_like(backbone_feats), t, event_diffused.float())
        
        elif cond_type == 'boundary05-':
            feature_mask = (boundary_gt < 0.5).float() # maybe try 0.1
            event_out, frame_feat = self.decoder(feature_mask * backbone_feats, t, event_diffused.float())
    ...
    def get_training_loss(self, video_feats, event_gt, boundary_gt,
        ...
        event_out, _ = self.forward(backbone_feats, t, event_diffused, event_gt, boundary_gt)
    ...
    def get_out(self, video_feats, event_gt, boundary_gt):

        if self.use_instance_norm:
            video_feats = self.ins_norm(video_feats)
        encoder_out, backbone_feats = self.encoder(video_feats, get_features=True)
        event_diffused, noise, t = self.prepare_targets(event_gt)
        event_out, frame_feat = self.forward(backbone_feats, t, event_diffused, event_gt, boundary_gt)
        return event_out, frame_feat
