...
class MS_TCN2(nn.Module):
    def __init__(self, num_layers_PG, num_layers_R, num_R, num_f_maps, dim, num_classes):
        ...
        self.PG = Prediction_Generation(num_layers_PG, num_f_maps, dim, num_classes) ## 11
        self.Rs = nn.ModuleList([copy.deepcopy(Refinement(num_layers_R, num_f_maps, num_classes, num_classes)) for s in range(num_R)]) ## 3

    def forward(self, x):
        out, out_feat = self.PG(x)
        outputs_feat = out_feat.unsqueeze(0)
        outputs = out.unsqueeze(0)
        for R in self.Rs:
            out, out_feat = R(F.softmax(out, dim=1))
            outputs = torch.cat((outputs, out.unsqueeze(0)), dim=0)
            outputs_feat = torch.cat((outputs_feat, out_feat.unsqueeze(0)), dim=0)

        return outputs, outputs_feat
...
class Refinement(nn.Module):
    def __init__(self, num_layers, num_f_maps, dim, num_classes):
        ...
        self.layers = nn.ModuleList([copy.deepcopy(DilatedResidualLayer(2**i, num_f_maps, num_f_maps)) for i in range(num_layers)]) ##10
        self.conv_out = nn.Conv1d(num_f_maps, num_classes, 1)
...
class Trainer:
    def __init__(self, num_layers_PG, num_layers_R, num_R, num_f_maps, dim, num_classes, dataset, split):
        ...
        logger.add("logs/" + dataset + "_" + str(split) + "_{time}.log")
