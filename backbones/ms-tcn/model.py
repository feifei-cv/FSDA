...
class MultiStageModel(nn.Module):
    ...
    def forward(self, x, mask):
        out, out_feat = self.stage1(x, mask)
        outputs_feat = out_feat.unsqueeze(0)
        outputs = out.unsqueeze(0)
        for s in self.stages:
            out, out_feat = s(F.softmax(out, dim=1) * mask[:, 0:1, :], mask)
...
class SingleStageModel(nn.Module):
    ...
    def forward(self, x, mask):
        ...
        for layer in self.layers:
            out_feat = layer(out_feat, mask)
        out = self.conv_out(out_feat) * mask[:, 0:1, :]
        return out, out_feat
