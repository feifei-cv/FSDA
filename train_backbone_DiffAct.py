...
if __name__ == '__main__':
    ...
    mapping_file = 'logs/train_backbone_DiffAct_' + time.ctime() + '.txt'
    sys.stdout = Logger(mapping_file)
    with open(mapping_file, 'a') as f:
        f.write('Begin training backbone DiffAct with 3090 GPU \n')
    ...
    for dataset in ['gtea', '50salads', 'breakfast']:
        for spl in ([1, 2, 3, 4, 5]):
            ...
            trainer.train(train_train_dataset, loss_weights, class_weighting, soft_label, num_epochs, batch_size,
                          learning_rate, weight_decay, result_dir=os.path.join(result_dir, naming),
                          save_dir=model_dir, log_freq=log_freq)
